...
## Highlights
- Most risk modeling studies report on outcome prevalence and discriminatory performance of the model (area under the receiver operating characteristic / c-statistic).
- Decision analytic modeling, sample size calculation, and value-of-information analysis for risk prediction models often require specification of the distribution of predicted risks.
- We show that for a wide class of probability distributions for risks, knowing the outcome prevalence and the model's c-statistic is enough to identify the distribution of risks.
...
library(tidyverse)
library(voipred) #Install from https://github.com/resplab/voipred
library(pROC)
library(WeightedROC)
library(sqldf)
library(mcmapper)
library(bayespmtools) #Install from https://github.com/resplab/bayespmtools
...
{
  w <- rgamma(n,1,1)
  tp.fp <- WeightedROC(dev_data$pi, dev_data$Y, w)
...
Overall, this approach provides objectivity to specifying the distribution of predicted risks, with applicability in health technology assessments, sample size calculations, and VoI analyses. Given the ubiquity of reporting prevalence and c-statistic in model development and validation studies, we expect this approach to play an important role in the future implementation and uptake of this approach.
